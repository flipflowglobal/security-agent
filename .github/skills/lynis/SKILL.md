---
name: lynis
description: Security auditing tool for Unix/Linux systems that checks local hardening, compliance, and configuration posture.
...
Security auditing tool for Unix/Linux systems that checks local hardening, compliance, and configuration posture.
...
Approved for the `CloudIaC`, `ContainerK8s`, `Compliance` specialist(s) in `src/registry.rs`.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `lynis` must
