# dirb — built-in offline substitute

**Real tool:** Web content directory brute-forcer.
...
## Usage
```
security-agent --run-tool dirb <input-file>
```
Runs entirely offline — no network access, and no external `dirb` binary is
ever spawned.
