# apktool — built-in offline substitute

**Real tool:** APK resource decoder/rebuilder.
...
## Usage
```
security-agent --run-tool apktool <input-file>
```
Runs entirely offline — no network access, and no external `apktool` binary is
ever spawned.
