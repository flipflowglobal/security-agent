# netdiscover — built-in offline substitute

**Real tool:** ARP-based local network host discovery.
...
## Usage
```
security-agent --run-tool netdiscover <input-file>
```
Runs entirely offline — no network access, and no external `netdiscover` binary is
ever spawned.
