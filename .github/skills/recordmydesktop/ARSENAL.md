# recordmydesktop — built-in offline substitute

**Real tool:** Desktop session recording (artifact) tool.
...
## Usage
```
security-agent --run-tool recordmydesktop <input-file>
```
Runs entirely offline — no network access, and no external `recordmydesktop` binary is
ever spawned.
