---
name: burpsuite
description: Integrated web-application security testing platform (intercepting proxy, scanner, repeater, intruder).
...
Integrated web-application security testing platform (intercepting proxy, scanner, repeater, intruder).
...
Approved for the `Dast`, `ApiSecurity` specialist(s) in `src/registry.rs`.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `burpsuite` must
