# burpsuite — built-in offline substitute

**Real tool:** Intercepting web proxy and application testing suite.
...
## Usage
```
security-agent --run-tool burpsuite <input-file>
```
Runs entirely offline — no network access, and no external `burpsuite` binary is
ever spawned.
