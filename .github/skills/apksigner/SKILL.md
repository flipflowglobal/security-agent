---
name: apksigner
description: Official Android tool that signs and verifies APKs against platform signature-scheme requirements.
...
Official Android tool that signs and verifies APKs against platform signature-scheme requirements.
...
Real execution is available: `--run-external-tool apksigner <args>` runs the locally installed binary directly (see `src/execution.rs`), bounded by an execution timeout with stdout/stderr/exit-code capture.

## Availability

Security-Agent never downloads, contacts, or silently invokes this tool. `apksigner` must
