# skipfish — built-in offline substitute

**Real tool:** Active web application reconnaissance scanner.
...
## Usage
```
security-agent --run-tool skipfish <input-file>
```
Runs entirely offline — no network access, and no external `skipfish` binary is
ever spawned.
