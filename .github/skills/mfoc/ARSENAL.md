# mfoc — built-in offline substitute

**Real tool:** MIFARE Classic key-recovery (NFC) tool.
...
## Usage
```
security-agent --run-tool mfoc <input-file>
```
Runs entirely offline — no network access, and no external `mfoc` binary is
ever spawned.
