---
name: mitmproxy
description: Interactive, HTTPS-capable intercepting proxy for inspecting and modifying HTTP(S) traffic in transit.
...
Interactive, HTTPS-capable intercepting proxy for inspecting and modifying HTTP(S) traffic in transit.
...
Approved for the `ApiSecurity` specialist(s) in `src/registry.rs`.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `mitmproxy` must
