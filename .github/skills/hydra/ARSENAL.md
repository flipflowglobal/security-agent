# hydra — built-in offline substitute

**Real tool:** Parallelized network login brute-forcer.
...
## Usage
```
security-agent --run-tool hydra <input-file>
```
Runs entirely offline — no network access, and no external `hydra` binary is
ever spawned.
