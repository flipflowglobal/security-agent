# ffuf — built-in offline substitute

**Real tool:** Fast web fuzzer for content/parameter discovery.
...
## Usage
```
security-agent --run-tool ffuf <input-file>
```
Runs entirely offline — no network access, and no external `ffuf` binary is
ever spawned.
