# netexec — built-in offline substitute

**Real tool:** Network execution and credential-spraying toolkit (CrackMapExec successor).
...
## Usage
```
security-agent --run-tool netexec <input-file>
```
Runs entirely offline — no network access, and no external `netexec` binary is
ever spawned.
