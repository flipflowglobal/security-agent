---
name: wireshark
description: Network protocol analyzer for capturing and inspecting packet traffic. Security-Agent ships an offline built-in substitute that only parses local classic PCAP files (no live capture).
...
Network protocol analyzer for capturing and inspecting packet traffic. Security-Agent ships an offline built-in substitute that only parses local classic PCAP files (no live capture).
...
Security-Agent never downloads, contacts, or silently invokes this tool. `wireshark` must
