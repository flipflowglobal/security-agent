# wafw00f — built-in offline substitute

**Real tool:** Web application firewall detector.
...
## Usage
```
security-agent --run-tool wafw00f <input-file>
```
Runs entirely offline — no network access, and no external `wafw00f` binary is
ever spawned.
