---
name: enum4linux
description: Enumerates information from Windows/Samba hosts via SMB (users, shares, groups, password policy).
...
Enumerates information from Windows/Samba hosts via SMB (users, shares, groups, password policy).
...
Security-Agent never downloads, contacts, or silently invokes this tool. `enum4linux` must
