# enum4linux — built-in offline substitute

**Real tool:** SMB/Windows enumeration tool.
...
## Usage
```
security-agent --run-tool enum4linux <input-file>
```
Runs entirely offline — no network access, and no external `enum4linux` binary is
ever spawned.
