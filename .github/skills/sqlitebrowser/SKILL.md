---
name: sqlitebrowser
description: GUI/CLI tool (DB Browser for SQLite) for inspecting and querying local SQLite database files.
...
GUI/CLI tool (DB Browser for SQLite) for inspecting and querying local SQLite database files.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `sqlitebrowser` must
