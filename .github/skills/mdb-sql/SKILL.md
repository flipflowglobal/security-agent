---
name: mdb-sql
description: Runs SQL queries against Microsoft Access (.mdb/.accdb) database files without requiring Access to be installed.
...
Runs SQL queries against Microsoft Access (.mdb/.accdb) database files without requiring Access to be installed.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `mdb-sql` must
