# mdb-sql — built-in offline substitute

**Real tool:** Microsoft Access (.mdb) database reader.
...
## Usage
```
security-agent --run-tool mdb-sql <input-file>
```
Runs entirely offline — no network access, and no external `mdb-sql` binary is
ever spawned.
