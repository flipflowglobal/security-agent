---
name: nuclei
description: Template-driven vulnerability scanner that sends crafted requests to detect known CVEs and misconfigurations.
...
Template-driven vulnerability scanner that sends crafted requests to detect known CVEs and misconfigurations.
...
Approved for the `Dast`, `ApiSecurity`, `MobileAndroid` specialist(s) in `src/registry.rs`.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `nuclei` must
