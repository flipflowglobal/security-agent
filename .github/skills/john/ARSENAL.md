# john — built-in offline substitute

**Real tool:** John the Ripper — multi-format password hash cracker.
...
## Usage
```
security-agent --run-tool john <input-file>
```
Runs entirely offline — no network access, and no external `john` binary is
ever spawned.
