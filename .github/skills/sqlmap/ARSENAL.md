# sqlmap — built-in offline substitute

**Real tool:** Automatic SQL-injection detection and exploitation.
...
## Usage
```
security-agent --run-tool sqlmap <input-file>
```
Runs entirely offline — no network access, and no external `sqlmap` binary is
ever spawned.
