# ike-scan — built-in offline substitute

**Real tool:** IKE/IPsec VPN discovery and fingerprinting.
...
## Usage
```
security-agent --run-tool ike-scan <input-file>
```
Runs entirely offline — no network access, and no external `ike-scan` binary is
ever spawned.
