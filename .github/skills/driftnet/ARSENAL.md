# driftnet — built-in offline substitute

**Real tool:** Image/media extractor from network traffic.
...
## Usage
```
security-agent --run-tool driftnet <input-file>
```
Runs entirely offline — no network access, and no external `driftnet` binary is
ever spawned.
