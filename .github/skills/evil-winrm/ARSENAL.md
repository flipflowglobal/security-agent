# evil-winrm — built-in offline substitute

**Real tool:** WinRM remote shell for authenticated Windows access.
...
## Usage
```
security-agent --run-tool evil-winrm <input-file>
```
Runs entirely offline — no network access, and no external `evil-winrm` binary is
ever spawned.
