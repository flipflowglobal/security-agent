# mfterm — built-in offline substitute

**Real tool:** MIFARE card interaction terminal (NFC).
...
## Usage
```
security-agent --run-tool mfterm <input-file>
```
Runs entirely offline — no network access, and no external `mfterm` binary is
ever spawned.
