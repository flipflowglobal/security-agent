# hashcat — built-in offline substitute

**Real tool:** GPU-accelerated password hash cracker.
...
## Usage
```
security-agent --run-tool hashcat <input-file>
```
Runs entirely offline — no network access, and no external `hashcat` binary is
ever spawned.
