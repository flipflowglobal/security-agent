# tcpdump — built-in offline substitute

**Real tool:** Command-line packet capture and analysis.
...
## Usage
```
security-agent --run-tool tcpdump <input-file>
```
Runs entirely offline — no network access, and no external `tcpdump` binary is
ever spawned.
