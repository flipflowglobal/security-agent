# ophcrack — built-in offline substitute

**Real tool:** Rainbow-table Windows LM/NTLM hash cracker.
...
## Usage
```
security-agent --run-tool ophcrack <input-file>
```
Runs entirely offline — no network access, and no external `ophcrack` binary is
ever spawned.
