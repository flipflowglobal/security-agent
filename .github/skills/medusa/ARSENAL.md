# medusa — built-in offline substitute

**Real tool:** Parallel modular network brute-forcer.
...
## Usage
```
security-agent --run-tool medusa <input-file>
```
Runs entirely offline — no network access, and no external `medusa` binary is
ever spawned.
