---
name: apkleaks
description: Scans a decompiled APK for accidentally embedded secrets, API keys, and URIs.
...
Scans a decompiled APK for accidentally embedded secrets, API keys, and URIs.
...
Approved for the `Secrets`, `MobileAndroid` specialist(s) in `src/registry.rs`.
...
Security-Agent never downloads, contacts, or silently invokes this tool. `apkleaks` must
