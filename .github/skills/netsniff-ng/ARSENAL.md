# netsniff-ng — built-in offline substitute

**Real tool:** High-performance packet capture toolkit.
...
## Usage
```
security-agent --run-tool netsniff-ng <input-file>
```
Runs entirely offline — no network access, and no external `netsniff-ng` binary is
ever spawned.
