# nikto — built-in offline substitute

**Real tool:** Web-server vulnerability and misconfiguration scanner.
...
## Usage
```
security-agent --run-tool nikto <input-file>
```
Runs entirely offline — no network access, and no external `nikto` binary is
ever spawned.
