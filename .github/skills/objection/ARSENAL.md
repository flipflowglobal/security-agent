# objection — built-in offline substitute

**Real tool:** Runtime mobile exploration (Frida-based).
...
## Usage
```
security-agent --run-tool objection <input-file>
```
Runs entirely offline — no network access, and no external `objection` binary is
ever spawned.
