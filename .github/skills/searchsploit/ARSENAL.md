# searchsploit — built-in offline substitute

**Real tool:** Offline Exploit-DB search.
...
## Usage
```
security-agent --run-tool searchsploit <input-file>
```
Runs entirely offline — no network access, and no external `searchsploit` binary is
ever spawned.
