---
name: jadx
description: Decompiles Android DEX bytecode back into readable Java source for static analysis.
...
Decompiles Android DEX bytecode back into readable Java source for static analysis.
...
Real execution is available: `--run-external-tool jadx <args>` runs the locally installed binary directly (see `src/execution.rs`), bounded by an execution timeout with stdout/stderr/exit-code capture.

## Availability

Security-Agent never downloads, contacts, or silently invokes this tool. `jadx` must
