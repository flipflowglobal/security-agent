...
//! ```
//!
//! Authorization fields are mandatory and fail-closed: missing
//! `engagement_id`, `authorized_by`, `authorized_by_role`,
//! `time_window_start`, `time_window_end`, `max_intensity`,
//! `high_impact_approved`, or `penetrative_testing_approved` is a parse
//! error, and the policy layer (`crate::policy`) enforces every field at
//! planning time. Malformed `key=value` lines, empty `[target]` sections,
//! and invalid enum/number values also error so typos are caught.
//!
...
";
    ...
    #[test]
    fn rejects_config_without_authorization_fields() {
        // An empty or authorization-free config must fail closed rather
        // than planning an unrestricted engagement.
        let result = parse_engagement_config("");
        assert!(matches!(
            result,
            Err(EngagementConfigError::MissingField(_))
        ));
    }
